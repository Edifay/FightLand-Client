...
public class Communication {
	 ...
	 * 
	 * @param sender:         -TCP (client): null -UDP (server): need a packet get
	 *                        from the receiver : get all first packet receive from
	 *                        clients : (ArrayList type DatagramPacket)
	 *                        this.getUDPClient() -UDP (client): null
	 * 
	 * @param connectionType: -TCP (client): Communication.TCP -UDP (client/server):
	 *                        Communication.UDP
	 * 
	 * @param pack:           - all : your Packet
	 */
	 ...
	 * 
	 * @param connectionType : -TCP : Communication.TCP -UDP : Communication.UDP
	 * 
	...
	public Packet nextPacket(long connectionType) {
		...
		if (connectionType == TCP) {
			...
			throw new IllegalArgumentException("Argument Possible : " + SERVER + "/" + CLIENT);
		}
		return pack;
	}
